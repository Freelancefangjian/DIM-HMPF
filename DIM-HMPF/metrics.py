...
def sam(img1, img2):
    ...
    inner_product = (img1_ * img2_).sum(axis=2)+ np.finfo(np.float64).eps
    img1_spectral_norm = np.sqrt((img1_**2).sum(axis=2))+ np.finfo(np.float64).eps
    img2_spectral_norm = np.sqrt((img2_**2).sum(axis=2))+ np.finfo(np.float64).eps
    # numerical stability
    cos_theta = (inner_product / (img1_spectral_norm * img2_spectral_norm)).clip(min=0, max=1)
    return np.mean(np.arccos(cos_theta))*180/math.pi
...
def metric(data_get, data_GT):

    data_get = data_get.cpu().detach().numpy()
    data_get = np.transpose(data_get, [0, 2, 3, 1])
    data_get = np.reshape(data_get, (512, 512, 128))
    data_get = im2uint8(data_get)


    data_GT = data_GT.cpu().detach().numpy()
    data_GT = np.transpose(data_GT, [0, 2, 3, 1])
    data_GT = np.reshape(data_GT, (512, 512, 128))
    data_GT = im2uint8(data_GT)

    psnr = psnr_HSI(data_get, data_GT)
    rmse_HSI = rmse(data_get, data_GT)
    ergas_HSI = ergas(data_get, data_GT, scale=8)
    sam_HSI = sam(data_get, data_GT)
    uiqi = qindex(data_get, data_GT, block_size=32)
    ssim_HSI = ssim(data_get, data_GT)
    return psnr, rmse_HSI, ergas_HSI, sam_HSI, uiqi, ssim_HSI, data_get
